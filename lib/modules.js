...
function xmlHTTPRquest(options) {
  ...
  if (loadingType && loadingType)

    if (!options.loadingType || options.loadingType === 'toast') {
      wx.showToast({
        title: "正在加载",
        icon: 'loading'
      })
    } else if (options.loadingType === 'navigation'){
        ...
        if (data.error) {
          ...
          if (options.success && typeof options.success === 'function') {
            options.success(data.data)
					...
					options.success()  
					//console.warn('返回体中既没有 error， 也没有 data， 与约定不符合')   
        }
      },
      fail (error) {
        // 服务端无法处理的错误
